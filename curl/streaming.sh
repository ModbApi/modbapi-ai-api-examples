...
# --no-buffer lets you see each SSE event as soon as the gateway sends it.
curl --fail-with-body --no-buffer --silent --show-error \
  ...
  --data "{\"model\":\"$MODB_MODEL\",\"messages\":[{\"role\":\"user\",\"content\":\"Count from one to three.\"}],\"stream\":true,\"max_tokens\":64}"
